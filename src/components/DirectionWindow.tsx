  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4 pt-2">
            ...
            const rowContent = (
              <>
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="font-semibold text-sm text-white group-hover:text-amber-200 transition-colors flex items-center gap-2">
                    <span className="text-zinc-200">{item.artist}</span>
                    <span className="text-zinc-500">-</span>
                    <span className="text-white group-hover:underline decoration-amber-400/50 underline-offset-4">‘{item.title}’</span>
                  </div>
                </div>
            ...
            if (item.url) {
              return (
                <a
                  ...
                  aria-label={`${item.artist} - ${item.title} 음원 듣기`}
                  className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 hover:bg-white/10 active:bg-white/15 transition-all group cursor-pointer block text-inherit no-underline"
                >
                  {rowContent}
                </a>
              );
        ...
        {/* 11. Footnote at the bottom of the credits screenshot area */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 px-1 pt-1 text-xs text-zinc-400 font-sans">
          <p className="flex items-center gap-1.5 text-zinc-300 font-medium">
            <span className="text-amber-400 font-bold">*</span>
            <span>이외 30곡 이상 참여</span>
          </p>
          <span className="text-[11px] text-zinc-500">
            각 트랙을 클릭하시면 공식 스트리밍(Apple Music, YouTube, SoundCloud)으로 연결됩니다.
          </span>
        </div>
      </div>
